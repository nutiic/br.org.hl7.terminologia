ValueSet: BREstrategiaVacinacao
Id: BREstrategiaVacinacao
Title: "BR Estrategia Vacinacao"
* ^version = "1.1.0"
* ^status = #active
* ^date = "2026-03-09T15:10:29-03:00"
* ^publisher = "Ministério do Saúde do Brasil"
* ^contact[0].name = "Ministério do Saúde do Brasil"
* ^contact[=].telecom[0].system = #url
* ^contact[=].telecom[=].value = "https://gov.br/saude"
* ^contact[=].telecom[+].system = #email
* ^contact[=].telecom[=].value = "contato@gointerop.com"
* ^contact[+].name = "Italo Macedo"
* ^contact[=].telecom.system = #email
* ^contact[=].telecom.value = "italo@gointerop.com"
* ^contact[=].telecom.use = #work
* ^jurisdiction = urn:iso:std:iso:3166#BR "Brazil"
* BREstrategiaVacinacao#3 "Bloqueio"
* BREstrategiaVacinacao#2 "Especial"
* BREstrategiaVacinacao#9 "Monitoramento rápido de cobertura vacinal"
* BREstrategiaVacinacao#13 "Reexposição"
* BREstrategiaVacinacao#8 "Serviço Privado"
* BREstrategiaVacinacao#12 "Pós-exposição"
* BREstrategiaVacinacao#7 "Soroterapia"
* BREstrategiaVacinacao#11 "Pré-exposição"
* BREstrategiaVacinacao#6 "Campanha seletiva"
* BREstrategiaVacinacao#10 "Pesquisa"
* BREstrategiaVacinacao#5 "Campanha indiscriminada"
* BREstrategiaVacinacao#4 "Intensificação"
* BREstrategiaVacinacao#1 "Rotina"
