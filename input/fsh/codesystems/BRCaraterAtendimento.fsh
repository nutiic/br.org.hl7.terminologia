CodeSystem: BRCaraterAtendimento
Id: BRCaraterAtendimento
Title: "BRCaraterAtendimento"
* ^meta.lastUpdated = "2025-06-02T14:55:21.222+00:00"
* ^version = "1.1.0"
* ^status = #active
* ^experimental = false
* ^date = "2026-03-09T15:10:29-03:00"
* ^publisher = "Ministério do Saúde do Brasil"
* ^contact[0].name = "Ministério do Saúde do Brasil"
* ^contact[=].telecom[0].system = #url
* ^contact[=].telecom[=].value = "https://gov.br/saude"
* ^contact[=].telecom[+].system = #email
* ^contact[=].telecom[=].value = "contato@gointerop.com"
* ^contact[+].name = "Italo Macedo"
* ^contact[=].telecom.system = #email
* ^contact[=].telecom.value = "italo@gointerop.com"
* ^contact[=].telecom.use = #work
* ^jurisdiction = urn:iso:std:iso:3166#BR "Brazil"
* ^caseSensitive = true
* ^content = #complete
* #99 "Sem registro no modelo de informação de origem"
* #06 "Demanda espontânea (DE): atendimento de urgência"
* #05 "Demanda espontânea: consulta no dia"
* #04 "Consulta agendada programada: cuidado continuado"
* #03 "Consulta agendada"
* #02 "Urgência"
* #01 "Eletivo"
