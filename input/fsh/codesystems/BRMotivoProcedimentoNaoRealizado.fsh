CodeSystem: BRMotivoProcedimentoNaoRealizado
Id: BRMotivoProcedimentoNaoRealizado
Title: "BRMotivoProcedimentoNaoRealizado"
* ^meta.lastUpdated = "2025-07-17T17:27:12.975+00:00"
* ^version = "1.1.0"
* ^status = #active
* ^experimental = false
* ^date = "2026-03-09T15:10:29-03:00"
* ^publisher = "Ministério do Saúde do Brasil"
* ^contact[0].name = "Ministério do Saúde do Brasil"
* ^contact[=].telecom[0].system = #url
* ^contact[=].telecom[=].value = "https://gov.br/saude"
* ^contact[=].telecom[+].system = #email
* ^contact[=].telecom[=].value = "contato@gointerop.com"
* ^contact[+].name = "Italo Macedo"
* ^contact[=].telecom.system = #email
* ^contact[=].telecom.value = "italo@gointerop.com"
* ^contact[=].telecom.use = #work
* ^jurisdiction = urn:iso:std:iso:3166#BR "Brazil"
* ^caseSensitive = true
* ^content = #complete
* #08 "Procedimento não concluído"
* #06 "Procedimento não desejado"
* #05 "Procedimento não oferecido"
* #04 "Procedimento não indicado"
* #03 "Procedimento não realizado"
* #02 "Procedimento descontinuado"
* #01 "Procedimento contraindicado"
