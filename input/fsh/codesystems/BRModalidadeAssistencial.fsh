CodeSystem: BRModalidadeAssistencial
Id: BRModalidadeAssistencial
Title: "BRModalidadeAssistencial"
* ^meta.lastUpdated = "2025-07-17T17:40:29.893+00:00"
* ^version = "1.1.0"
* ^status = #active
* ^experimental = false
* ^date = "2026-03-09T15:10:29-03:00"
* ^publisher = "Ministério do Saúde do Brasil"
* ^contact[0].name = "Ministério do Saúde do Brasil"
* ^contact[=].telecom[0].system = #url
* ^contact[=].telecom[=].value = "https://gov.br/saude"
* ^contact[=].telecom[+].system = #email
* ^contact[=].telecom[=].value = "contato@gointerop.com"
* ^contact[+].name = "Italo Macedo"
* ^contact[=].telecom.system = #email
* ^contact[=].telecom.value = "italo@gointerop.com"
* ^contact[=].telecom.use = #work
* ^jurisdiction = urn:iso:std:iso:3166#BR "Brazil"
* ^caseSensitive = true
* ^content = #complete
* #05 "Atenção Psicossocial"
* #01 "Atenção Básica"
* #06 "Atenção à Urgência/Emergência"
* #07 "Ambulatorial Especializada"
* #02 "Atenção Domiciliar"
* #03 "Atenção Intermediária"
* #08 "Assistência Farmacêutica"
* #04 "Atenção Hospitalar"
* #09 "Assistência Ambulatorial"
